...
module.exports = {
        ...
        onStart: async function ({ api, event, args, message, getLang }) {
                ...
                try {
                        ...
                        if (args[0] === "add") {
                                ...
                                const res = await axios.post(`${apiBase}/api/album2/abdullah/add`, { category: args[1].toLowerCase(), videoUrl: imgurRes.data.link });
                                api.setMessageReaction("🪽", event.messageID, () => {}, true);
                        ...
                        if (args[0] === "list") {
                                api.setMessageReaction("⏳", event.messageID, () => {}, true);
                                const res = await axios.get(`${apiBase}/api/album2/abdullah/list`);
                                api.setMessageReaction("🪽", event.messageID, () => {}, true);
                                return message.reply(res.data.message);
                        }

                        api.setMessageReaction("⏳", event.messageID, () => {}, true);
                        const configRes = await axios.get(`${apiBase}/api/album2/abdulla/display`);
        ...
        onReply: async function ({ api, event, Reply, getLang, message }) {
                ...
                try {
                        api.setMessageReaction("⏳", event.messageID, () => {}, true);
                        const apiBase = await mahmud();
                        const response = await axios.get(`${apiBase}/api/album2/abdulla/videos/${category}?userID=${event.senderID}`);
